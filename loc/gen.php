...
// Check if the logout query parameter is set, to allow logging out
if (isset($_GET['logout'])) {
    unset($_SESSION['authenticated']);
    header('Location: ' . $_SERVER['PHP_SELF']); // Redirect to the same script without 'logout' parameter
    exit;
}

// The plain text password - highly discouraged for actual use
$plainTextPassword = 'STEVEN TAMPAN DAN PEMBERANI LOH KANJUT';

// Check if the login form was submitted and the password is correct
if (isset($_POST['password']) && $_POST['password'] === $plainTextPassword) {
    $_SESSION['authenticated'] = true; // Set the session as authenticated
    header('Location: ' . $_SERVER['PHP_SELF']); // Optionally, redirect to clear the POST request
    exit;
}

// If the session is not authenticated, display the login form
if (!isset($_SESSION['authenticated'])) {
    // Display the login form
    echo '<!DOCTYPE html><html><head><title>Login Dulu Kontol</title></head><body>';
    if (isset($_POST['password'])) {
        // This means a password was submitted but it was incorrect
